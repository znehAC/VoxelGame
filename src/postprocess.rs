...
impl BloomPipeline {
    pub fn new(gpu: &GpuContext, width: u32, height: u32, output_format: wgpu::TextureFormat) -> Self {
        ...
        let mut pipeline = Self {
            ...
            hdr_texture: create_texture(device, 1, 1, wgpu::TextureFormat::Rgba16Float, "Placeholder"),
            hdr_view: create_texture(device, 1, 1, wgpu::TextureFormat::Rgba16Float, "Placeholder").create_view(&Default::default()),
            bloom_texture: create_texture(device, 1, 1, wgpu::TextureFormat::Rgba16Float, "Placeholder"),
            bloom_view: create_texture(device, 1, 1, wgpu::TextureFormat::Rgba16Float, "Placeholder").create_view(&Default::default()),
            blur_temp_texture: create_texture(device, 1, 1, wgpu::TextureFormat::Rgba16Float, "Placeholder"),
            blur_temp_view: create_texture(device, 1, 1, wgpu::TextureFormat::Rgba16Float, "Placeholder").create_view(&Default::default()),
        };
    ...
    pub fn resize(&mut self, gpu: &GpuContext, width: u32, height: u32) {
        ...
        self.hdr_texture = create_texture(device, width, height, wgpu::TextureFormat::Rgba16Float, "HDR Scene");
        ...
        self.bloom_texture = create_texture(device, bloom_width, bloom_height, wgpu::TextureFormat::Rgba16Float, "Bloom A");
        self.bloom_view = self.bloom_texture.create_view(&Default::default());

        self.blur_temp_texture = create_texture(device, bloom_width, bloom_height, wgpu::TextureFormat::Rgba16Float, "Bloom B");
        self.blur_temp_view = self.blur_temp_texture.create_view(&Default::default());
    }

    pub fn render(&self, device: &wgpu::Device, encoder: &mut wgpu::CommandEncoder, view: &wgpu::TextureView) {
        // 1. Threshold: HDR -> Bloom A
        {
            ...
            pass.set_push_constants(wgpu::ShaderStages::FRAGMENT, 0, bytemuck::bytes_of(&threshold));
            pass.draw(0..3, 0..1);
        }
        ...
        // 4. Composite: HDR + Bloom A -> Swapchain
        {
            ...
            let params = CompositeParams { intensity: 0.8, exposure: 1.0 };
            pass.set_push_constants(wgpu::ShaderStages::FRAGMENT, 0, bytemuck::bytes_of(&[params.intensity, params.exposure]));
            pass.draw(0..3, 0..1);
        }
    }
