...
/// Load the voxel raytracer shader from `assets/shaders/voxel_raytracer.wgsl`.
///
/// Returns the shader source as a string, or an error message on failure.
pub fn load_shader() -> Result<String, String> {
    let path = Path::new(ASSETS_DIR).join("shaders").join("voxel_raytracer.wgsl");
